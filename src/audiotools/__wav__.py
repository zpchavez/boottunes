...
from audiotools import AudioFile,InvalidFile,ChannelMask,PCMReader,construct,BUFFER_SIZE,transfer_data,__capped_stream_reader__,FILENAME_FORMAT,BIN,open_files,os,subprocess,cStringIO,EncodingError,DecodingError,UnsupportedChannelMask
import os.path
import gettext
from . import pcm
...
def __blank_channel_mask__():
    c = construct.Container(undefined=0,undefined2=0)
...
class WaveAudio(AudioFile):
    ...
    WAVE_HEADER = construct.Struct("wave_header",
                             construct.Const(construct.Bytes("wave_id",4),'RIFF'),
                             construct.ULInt32("wave_size"),
                             construct.Const(construct.Bytes("riff_type",4),'WAVE'))

    CHUNK_HEADER = construct.Struct("chunk_header",
                              construct.Bytes("chunk_id",4),
                              construct.ULInt32("chunk_length"))

    FMT_CHUNK = construct.Struct("fmt_chunk",
                           construct.ULInt16("compression"),
                           construct.ULInt16("channels"),
                           construct.ULInt32("sample_rate"),
                           construct.ULInt32("bytes_per_second"),
                           construct.ULInt16("block_align"),
                           construct.ULInt16("bits_per_sample"),
                           construct.If(lambda ctx: ctx['compression'] == 0xFFFE,
                                  construct.Embed(
                construct.Struct('extensible',
                           construct.ULInt16('cb_size'),
                           construct.ULInt16('valid_bits_per_sample'),
                           construct.BitStruct('channel_mask',
                                         #0x80
                                         construct.Flag('front_right_of_center'),

                                         #0x40
                                         construct.Flag('front_left_of_center'),

                                         #0x20
                                         construct.Flag('rear_right'),

                                         #0x10
                                         construct.Flag('rear_left'),

                                         #0x8
                                         construct.Flag('LFE'),

                                         #0x4
                                         construct.Flag('front_center'),

                                         #0x2
                                         construct.Flag('front_right'),

                                         #0x1
                                         construct.Flag('front_left'),

                                         #0x8000
                                         construct.Flag('top_back_left'),

                                         #0x4000
                                         construct.Flag('top_front_right'),

                                         #0x2000
                                         construct.Flag('top_front_center'),

                                         #0x1000
                                         construct.Flag('top_front_left'),

                                         #0x800
                                         construct.Flag('top_center'),

                                         #0x400
                                         construct.Flag('side_right'),

                                         #0x200
                                         construct.Flag('side_left'),

                                         #0x100
                                         construct.Flag('rear_center'),
                                         ...
                                         #0x80000
                                         #0x40000
                                         construct.Bits('undefined',6),

                                         #0x20000
                                         construct.Flag('top_back_right'),

                                         #0x10000
                                         construct.Flag('top_back_center'),

                                         construct.Bits('undefined2',8)
                                         ),
                           construct.String('sub_format',16)))
                                  )
                           )
    ...
    @classmethod
    def from_pcm_to_stream(cls, pcmreader, stream, compression=None):
        """
        Added for BootTunes.

        Write stream to pyaudio.Stream object instead of saving to a file, thus
        ...
        try:            
            ...
            fmt_header = construct.Container()
            fmt_header.chunk_id = 'fmt '

            fmt = construct.Container()
