...
def __strip_list_dict__(d,log):
    for key in list(d.keys())[:]:
        old_values = d[key]
        new_values = [v for v in old_values if (len(unicode(v).strip().strip(unichr(0))) > 0)]
...
def fix_track_number(metadata, log):
    if ((metadata.track_number >= 100) and
        ...
        log.append(_(u"Converted track number %(old_tracknum)d to album number %(albumnum)d and track_number %(new_tracknum)d") % \
        ...
        metadata.track_number = metadata.track_number % 100
          ...
          (metadata.track_number <= 999) and
          (metadata.album_number == (metadata.track_number / 100))):
        log.append(_(u"Converted track number %(old_tracknum)d to track number %(new_tracknum)d") % \
                   {"old_tracknum":metadata.track_number,
                    "new_tracknum":metadata.track_number % 100})
        metadata.track_number = metadata.track_number % 100
    ...
    if (isinstance(metadata,audiotools.VorbisComment)):
        ...
        for key in list(metadata.keys())[:]:
            if ((key in ("Track","Media")) and
                ...
                (unicode(metadata[key]).lstrip(u"0") != unicode(metadata[key]))):
...
def fix_whitespace(metadata, log):
    for field in TEXT_FIELDS:
        if (getattr(metadata,field).strip().strip(unichr(0)) !=
            ...
            setattr(metadata,field,getattr(metadata,field).strip().strip(unichr(0)))
...
def update_and_backup(track, undo_db, messenger):
    ...
    if (changes_made):
        ...
        if (subprocess.call([audiotools.BIN["xdelta"],
                             "delta",
                             temp_track.filename,
                             track.filename,
                             delta_f.name]) != 2):
