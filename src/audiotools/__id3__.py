...
from audiotools import MetaData,construct,re,os,cStringIO,Image,InvalidImage
import codecs
import gettext
...
class Syncsafe32(construct.Adapter):
    def __init__(self, name):
        construct.Adapter.__init__(self,
                             construct.StrictRepeater(4,construct.UBInt8(name)))
...
class __24BitsBE__(construct.Adapter):
    def _encode(self, value, context):
...
def UBInt24(name):
    return __24BitsBE__(construct.Bytes(name,3))
...
class WidecharCStringAdapter(construct.Adapter):
    def __init__(self,obj,encoding):
        construct.Adapter.__init__(self,obj)
        self.encoding = encoding

    def _encode(self,obj,context):
        return construct.GreedyRepeater(construct.UBInt16("s")).parse(obj.encode(
                self.encoding)) + [0]

    def _decode(self,obj,context):
        c = construct.UBInt16("s")

        return "".join([c.build(s) for s in obj[0:-1]]).decode(self.encoding)

def UCS2CString(name):
    return WidecharCStringAdapter(construct.RepeatUntil(lambda obj, ctx: obj == 0x0,
                                                  construct.UBInt16(name)),
                                  encoding='ucs2')
...
class ID3v22Frame:
    FRAME = construct.Struct("id3v22_frame",
                       construct.Bytes("frame_id",3),
                       construct.PascalString("data",length_field=UBInt24("size")))
    ...
    def build(self):
        return self.FRAME.build(construct.Container(frame_id=self.id,
                                              data=self.data))
...
class ID3v22TextFrame(ID3v22Frame):
    ...
    def build(self):
        return self.FRAME.build(construct.Container(
                frame_id=self.id,
...
class ID3v22ComFrame(ID3v22TextFrame):
    COMMENT_HEADER = construct.Struct(
        "com_frame",
        construct.Byte("encoding"),
        construct.String("language",3),
        construct.Switch("short_description",
                   lambda ctx: ctx.encoding,
                   {0x00: construct.CString("s",encoding='latin-1'),
                    0x01: UCS2CString("s")}))
    ...
    def build(self):
        return self.FRAME.build(construct.Container(
                frame_id=self.id,
                data=self.COMMENT_HEADER.build(construct.Container(
...
class ID3v22PicFrame(ID3v22Frame,Image):
    FRAME_HEADER = construct.Struct('pic_frame',
                              construct.Byte('text_encoding'),
                              construct.String('format',3),
                              construct.Byte('picture_type'),
                              construct.Switch("description",
                                         lambda ctx: ctx.text_encoding,
                                         {0x00: construct.CString("s",
                                                            encoding='latin-1'),
                                          0x01: UCS2CString("s")}))
    ...
    def build(self):
        ...
        return ID3v22Frame.FRAME.build(
            construct.Container(frame_id='PIC',
                          data=self.FRAME_HEADER.build(
                    construct.Container(text_encoding=text_encoding,
...
class ID3v22Comment(MetaData):
    ...
    TAG_HEADER = construct.Struct("id3v22_header",
                            construct.Const(construct.Bytes("file_id",3),'ID3'),
                            construct.Byte("version_major"),
                            construct.Byte("version_minor"),
                            construct.Embed(construct.BitStruct("flags",
                                                    construct.Flag("unsync"),
                                                    construct.Flag("compression"),
                                                    construct.Padding(6))),
                            Syncsafe32("length"))
    ...
    def build(self):
        ...
        return self.TAG_HEADER.build(
            construct.Container(file_id='ID3',
                          version_major=0x02,
                          version_minor=0x00,
                          unsync=False,
                          compression=False,
...
class ID3v23Frame(ID3v22Frame):
    FRAME = construct.Struct("id3v23_frame",
                       construct.Bytes("frame_id",4),
                       construct.UBInt32("size"),
                       construct.Embed(construct.BitStruct("flags",
                                               construct.Flag('tag_alter'),
                                               construct.Flag('file_alter'),
                                               construct.Flag('read_only'),
                                               construct.Padding(5),
                                               construct.Flag('compression'),
                                               construct.Flag('encryption'),
                                               construct.Flag('grouping'),
                                               construct.Padding(5))),
                       construct.String("data",length=lambda ctx: ctx["size"]))
...
class ID3v23PicFrame(ID3v23Frame,Image):
    FRAME_HEADER = construct.Struct('apic_frame',
                              construct.Byte('text_encoding'),
                              construct.CString('mime_type'),
                              construct.Byte('picture_type'),
                              construct.Switch("description",
                                         lambda ctx: ctx.text_encoding,
                                         {0x00: construct.CString("s",
...
class ID3v23ComFrame(ID3v23TextFrame):
    ...
    def build(self):
        return ID3v23Frame.build(
            self,
            self.COMMENT_HEADER.build(construct.Container(
                    encoding=self.encoding,
                    language=self.language,
                    short_description=self.short_description)) + \
                self.content.encode(self.ENCODING[self.encoding],'replace'))
...
class ID3v23Comment(ID3v22Comment):
    ...
    TAG_HEADER = construct.Struct("id3v23_header",
                            construct.Const(construct.Bytes("file_id",3),'ID3'),
                            construct.Byte("version_major"),
                            construct.Byte("version_minor"),
                            construct.Embed(construct.BitStruct("flags",
                                                    construct.Flag("unsync"),
                                                    construct.Flag("extended"),
                                                    construct.Flag("experimental"),
                                                    construct.Flag("footer"),
                                                    construct.Padding(4))),
                            Syncsafe32("length"))
...
class ID3v24Frame(ID3v23Frame):
    FRAME = construct.Struct("id3v24_frame",
                       construct.Bytes("frame_id",4),
                       Syncsafe32("size"),
                       construct.Embed(construct.BitStruct("flags",
                                               construct.Padding(1),
    ...
    COMMENT_HEADER = construct.Struct(
        ...
        construct.Switch("short_description",
                   ...
                   {0x00: construct.CString("s",encoding='latin-1'),
                    0x01: UTF16CString("s"),
                    0x02: UTF16BECString("s"),
                    0x03: construct.CString("s",encoding='utf-8')}))
