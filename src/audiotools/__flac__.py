...
class FlacMetaDataBlock:
    ...
    def build_block(self, last=0):
        ...
        return FlacAudio.METADATA_BLOCK_HEADER.build(
            construct.Container(last_block=last,
...
class FlacPictureComment(Image):
    ...
    def build(self):
        ...
        return FlacAudio.PICTURE_COMMENT.build(
            construct.Container(type=self.flac_type,
...
class FlacCueSheet:
    CUESHEET = construct.Struct(
        "flac_cuesheet",
        construct.String("catalog_number",128),
        construct.UBInt64("lead_in_samples"),
        construct.Embed(construct.BitStruct("flags",
                                construct.Flag("is_cd"),
                                construct.Padding(7))), #reserved
        construct.Padding(258), #reserved
        construct.PrefixedArray(
            length_field=construct.Byte("count"),
            subcon=construct.Struct("cuesheet_tracks",
                              construct.UBInt64("track_offset"),
                              construct.Byte("track_number"),
                              construct.String("ISRC",12),
                              construct.Embed(construct.BitStruct("sub_flags",
                                                      construct.Flag("non_audio"),
                                                      construct.Flag("pre_emphasis"),
                                                      construct.Padding(6))),
                              construct.Padding(13),
                              construct.PrefixedArray(
                    length_field=construct.Byte("count"),
                    subcon=construct.Struct("cuesheet_track_index",
                                      construct.UBInt64("offset"),
                                      construct.Byte("point_number"),
    ...
    def converted(cls, sheet, total_frames, sample_rate=44100):
        ...
        def track_container(number,ISRC,indexes):
            ...
            return construct.Container(
                ...
                cuesheet_track_index=[construct.Container(
        ...
        return cls(construct.Container(
                ...
                cuesheet_tracks=[track_container(i + 1,
                                 ...
                                 [construct.Container(track_offset=total_frames,
                                                track_number=170,
                                                ISRC=chr(0) * 12,
                                                non_audio=False,
                                                pre_emphasis=False,
...
class FlacSeektable:
    SEEKTABLE = construct.GreedyRepeater(
            construct.Struct("seekpoint",
                       construct.UBInt64("first_sample_number"),
                       construct.UBInt64("first_byte_offset"),
                       construct.UBInt16("sample_count")))
...
class FlacAudio(AudioFile):
    ...
    METADATA_BLOCK_HEADER = construct.BitStruct("metadata_block_header",
                                          construct.Bit("last_block"),
                                          construct.Bits("block_type",7),
                                          construct.Bits("block_length",24))

    STREAMINFO = construct.Struct("flac_streaminfo",
                                 construct.UBInt16("minimum_blocksize"),
                                 construct.UBInt16("maximum_blocksize"),
                                 construct.Embed(construct.BitStruct("flags",
                                   construct.Bits("minimum_framesize",24),
                                   construct.Bits("maximum_framesize",24),
                                   construct.Bits("samplerate",20),
                                   construct.Bits("channels",3),
                                   construct.Bits("bits_per_sample",5),
                                   construct.Bits("total_samples",36))),
                                 construct.StrictRepeater(16,construct.Byte("md5")))

    PICTURE_COMMENT = construct.Struct("picture_comment",
                                 construct.UBInt32("type"),
                                 construct.PascalString("mime_type",
                                                  length_field=construct.UBInt32("mime_type_length")),
                                 construct.PascalString("description",
                                                  length_field=construct.UBInt32("description_length")),
                                 construct.UBInt32("width"),
                                 construct.UBInt32("height"),
                                 construct.UBInt32("color_depth"),
                                 construct.UBInt32("color_count"),
                                 construct.PascalString("data",
                                                  length_field=construct.UBInt32("data_length")))
    ...
    def __block_ids__(cls, flacfile):
        p = construct.Container(last_block=False,
                          block_type=None,
                          block_length=0)
...
class OggFlacAudio(FlacAudio):
    ...
    OGGFLAC_STREAMINFO = construct.Struct('oggflac_streaminfo',
                                    construct.Const(construct.Byte('packet_byte'),
                                              0x7F),
                                    construct.Const(construct.String('signature',4),
                                              'FLAC'),
                                    construct.Byte('major_version'),
                                    construct.Byte('minor_version'),
                                    construct.UBInt16('header_packets'),
                                    construct.Const(construct.String('flac_signature',4),
                                              'fLaC'),
                                    construct.Embed(
