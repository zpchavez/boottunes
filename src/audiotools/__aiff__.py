...
from audiotools import AudioFile,InvalidFile,construct,PCMReader,__capped_stream_reader__,PCMReaderError,transfer_data,DecodingError,EncodingError,ID3v22Comment,BUFFER_SIZE,ChannelMask,UnsupportedChannelMask,ReorderedPCMReader
...
class IEEE_Extended(construct.Adapter):
    def __init__(self,name):
        construct.Adapter.__init__(
            self,
            construct.Struct(name,
                       construct.Embed(construct.BitStruct(None,
                                               construct.Flag("signed"),
...
class AiffAudio(AudioFile):
    ...
    AIFF_HEADER = construct.Struct("aiff_header",
                             construct.Const(construct.Bytes("aiff_id",4),"FORM"),
    ...
    def from_pcm(cls, filename, pcmreader, compression=None):
        ...
        try:
            ...
            comm_chunk = construct.Container(channels=pcmreader.channels,
                                       total_sample_frames=0,
                                       sample_size=pcmreader.bits_per_sample,
                                       sample_rate=float(pcmreader.sample_rate))

            ssnd_header = construct.Container(chunk_id='SSND',
                                        chunk_length=0)
            ssnd_alignment = construct.Container(offset=0,
                                           blocksize=0)
            ...
            f.write(cls.CHUNK_HEADER.build(construct.Container(
                        chunk_id='COMM',
