...
from audiotools import AudioFile,WaveAudio,InvalidFile,PCMReader,construct,transfer_data,subprocess,BIN,MetaData,os,re,TempWaveReader,Image,cStringIO
import gettext
...
class ApeTag(MetaData):
    ...
    APEv2_FOOTER = construct.Struct("APEv2",
      construct.String("preamble",8),
    ...
    def __parse_image__(self,key,type):
        ...
        description = construct.CString(None).parse_stream(data).decode('utf-8',
                                                                  'replace')
        data = data.read()
    ...
    def add_image(self,image):
        if (image.type == 0):
            self['Cover Art (front)'] = self.ITEM.external(
                'Cover Art (front)',
                construct.CString(None).build(image.description.encode(
...
class ApeAudio(ApeTaggedAudio,AudioFile):
    ...
    FILE_HEAD = construct.Struct("ape_head",
                           construct.String('id',4),
                           construct.ULInt16('version'))

    #version >= 3.98
    APE_DESCRIPTOR = construct.Struct("ape_descriptor",
                                construct.ULInt16('padding'),
